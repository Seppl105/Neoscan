...
# Create a new sketch
sketchOutline = model.ConstrainedSketch(name='Sketch-Coil', sheetSize=r_i + totalWindings*len(materialWidths) + 5*len(materialWidths))
sketchOutline = sketchOutline.rectangle(point1=(yCoorStart, r_i), point2=(yCoorEnd, r_a))
...
part.BaseShell(sketchOutline=sketchOutline)
...
for winding in range(totalWindings):
    for i,width in enumerate(materialWidths):
        ...
        faceToPartition = part.faces.findAt(((yCoorMid, outerR + width/2, 0.0),)) # 3D point needed?
        part.PartitionFaceByShortestPath(faces=faceToPartition, point1= (yCoorStart, outerR + width), point2= (yCoorEnd, outerR + width)) #2D points needed?
        # assign a section to the newly created layer
        faceOfNewLayer = part.faces.findAt(((yCoorMid, outerR + width/2, 0.0)))
